...
! DEBUG
               write(*,*) '^^^ tag of drop converted to LPP',drops(idrop,rank)%element%id, &
               drops(idrop,rank)%element%vol,rank,drops(idrop,rank)%element%xc,&
               drops(idrop,rank)%element%yc,drops(idrop,rank)%element%zc
! END DEBUG
...
! DEBUG
            write(*,*) 'tag of drop removed',drops(idrop,rank)%element%id, &
               drops(idrop,rank)%element%vol,rank
! END DEBUG
      ...
      do idrop = 1,num_drop_merge(rank)
         ...
         if ( convertDropFlag ) then
            ...
            ! remove droplet vof structure
! DEBUG
            write(*,*) 'tag of drop_merge removed',drops_merge(idrop,rank)%element%id, &
               drops_merge(idrop,rank)%element%vol,rank,drops_merge(idrop,rank)%flag_center_mass
! END DEBUG
...
! DEBUG
               write(*,*) '*** tag of drop_merge converted to LPP',drops_merge(idrop,rank)%element%id, &
               drops_merge(idrop,rank)%element%vol,rank,drops_merge(idrop,rank)%element%xc,&
               drops_merge(idrop,rank)%element%yc,drops_merge(idrop,rank)%element%zc
! END DEBUG
   ...
   subroutine UpdateParticle
         
   end subroutine UpdateParticle
